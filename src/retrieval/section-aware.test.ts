...
async function strategies(docs: WikiDoc[]) {
...
test("pinned partition is boosted above an otherwise-equal page", async () => {
  ...
  const withPin = await strategies(
    base.map((d) => (d.path === "b.md" ? { ...d, pinned: true } : d))
  );
  ...
  const rank = (r: typeof unpinned, path: string) =>
    r.chunks.findIndex((c) => c.path === path);
