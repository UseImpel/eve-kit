...
export type LoadStats = {
  total: number;
  carried: number; // passages whose vectors came from sidecars
  backfilled: number; // passages we embedded on load
  format: "v2";
};
...
export async function loadReleaseIndex(opts: {
  ...
  for (const pageDoc of pageDocs) {
    ...
    const pageEmbedding =
      vectors[0] ?? (await embedder([embeddingInput(pageDoc)]))[0];
