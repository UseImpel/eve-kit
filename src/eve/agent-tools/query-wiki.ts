import { Retrieval, type RetrievalAnswer } from "../../retrieval/index.js";
...
export async function queryWiki(
  ...
  try {
    ...
    try {
      ...
      console.log(
        `[queryWiki] orgId=${orgId} durationMs=${durationMs} gated=true reason=missing_wiki`
      );
      return result;
    }

    // Run the retrieval with optional k and floor.
    const answer: RetrievalAnswer = await retrieval.query(query, {
      k: options?.k ?? 5,
      floor: options?.floor,
    });

    // Tag all chunks with orgId.
    const chunks: QueryWikiChunk[] = answer.chunks.map((chunk) => ({
      text: chunk.snippet || "",
      source: "wiki",
    ...
    console.log(
      `[queryWiki] orgId=${orgId} durationMs=${durationMs} gated=${answer.gate.gated} chunks=${chunks.length}`
    );
    ...
    const durationMs = Date.now() - startMs;
    console.error(
      `[queryWiki] orgId=${orgId} durationMs=${durationMs} error=${err instanceof Error ? err.message : String(err)}`
    );
    throw err;
  }
}
