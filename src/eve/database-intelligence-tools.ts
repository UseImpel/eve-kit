...
async function postDatabaseIntelligence(
  ctx: ToolContext,
  path: string,
...
export const databaseCatalogStatusTool = defineTool({
  description:
    "Show the immutable database-schema catalog snapshot attached to this run, including generation time, engines, environments, and reconciled counts. Use this first when database coverage is uncertain.",
...
export const databaseSearchTool = defineTool({
  description:
    "Search the attached live-system database catalog for tables, views, collections, routines, fields, indexes, and constraints. Returns schema metadata only, never row or document values.",
...
export const databaseDescribeTool = defineTool({
  description:
    "Describe one database object from the attached catalog, including columns or fields, data types, indexes, and constraints. Use database plus qualifiedName when names may repeat.",
...
export const databaseRelationsTool = defineTool({
  description:
    "Trace bounded incoming and outgoing database-schema relationships from one object, such as foreign keys, using the immutable catalog attached to this run.",
...
export const databaseCompareTool = defineTool({
  description:
    "Compare structural schema metadata between two databases in the attached catalog and report objects present on one side or structurally changed. This does not compare stored data.",
