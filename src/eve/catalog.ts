...
export const EVE_KIT_PROVIDERS: readonly EveKitProvider[] = [
  {
    ...
    envVars: [
      ...
      },
      {
        ...
        sensitive: false,
        purpose:
          "Base URL of the impel-identity service for centralized GitHub token resolution during workspace prep; only used when the dispatcher also sent a server-only v1 clientContext.identityRunToken assertion.",
      },
      {
  ...
  },
  {
    ...
    summary:
      "Read, search, symbol-context, impact, trace, diff-impact, and index-status tools backed by the multi-tenant Impel code-intelligence service. Tenant identity comes only from the signed run assertion in Eve session auth.",
    envVars: [
      {
  ...
  },
  {
    ...
    summary:
      "Catalog-status, search, describe, relationship, and structural comparison tools backed by immutable multi-tenant database-schema snapshots. Tenant and snapshot identity come only from the signed Eve run assertion.",
    envVars: [
      {
        name: "IMPEL_CODE_INTELLIGENCE_URL",
        required: true,
  ...
  },
  {
    ...
    summary:
      "AI SDK LanguageModelV4 provider for Impel's conventional gateway API. Eve owns the tool loop while Claude routes through Anthropic Messages and GPT/Codex routes through OpenAI Responses.",
    envVars: [
      {
      ...
      },
      {
        ...
        sensitive: true,
        purpose:
          "Signed per-run token fallback. Hosted Eve calls should prefer clientContext.runToken.",
      },
      {
        ...
        sensitive: true,
        purpose:
          "Static PAT for local pilots or break-glass use when no signed run token is available.",
      },
    ],
    setupSteps: [
  ...
  },
  {
    ...
    summary:
      "Compatibility shim for the pure-Eve Impel gateway model. Uses plain Anthropic Messages; Eve owns tools, sandbox policy, transcripts, and structured output with no implicit local-provider fallback.",
    envVars: [
      {
        name: "IMPEL_GATEWAY_URL",
        required: true,
  ...
  },
  {
    ...
    summary:
      "Compatibility shim for the pure-Eve OpenAI Responses path through impel-gateway. Forces store:false so Eve can replay encrypted reasoning and tool history.",
    envVars: [
      {
        name: "IMPEL_GATEWAY_URL",
        required: true,
