...
export type ImpelGatewayHeaders =
  | HeadersInit
  | (() => HeadersInit | Promise<HeadersInit>);

export type ImpelGatewayRunContextProvider =
  | ImpelGatewayRunContext
  | (() => ImpelGatewayRunContext | Promise<ImpelGatewayRunContext>);
...
export type ImpelGatewayPoolErrorCode =
  | "model_not_entitled"
  | "pool_exhausted"
  | "pool_rate_limited";
...
type PackedRunContextScan = {
  hasTrustedCurrentContext: boolean;
  currentContext: ImpelGatewayRunContext | null;
...
export function normalizeImpelGatewayModelId(modelId: string): string {
  const trimmed = modelId.trim();
  if (!trimmed) return trimmed;

  const withoutProvider = trimmed.replace(/^(?:anthropic|openai)\//i, "");
  return (
    IMPEL_GATEWAY_MODEL_ALIASES[
      withoutProvider.toLowerCase() as keyof typeof IMPEL_GATEWAY_MODEL_ALIASES
    ] ?? withoutProvider
  );
}

export function resolveImpelGatewayModel(modelId: string): ResolvedGatewayModel {
  ...
  throw new Error(
    `Unsupported Impel gateway model id "${modelId}". Use a Claude, GPT, o-series, or Codex model id.`,
  );
}
 ...
 */
export function impelGatewayModel(
  modelId: string,
  options: ImpelGatewayModelOptions = {},
): LanguageModelV4 {
 ...
 */
export class ImpelGatewayLanguageModel implements LanguageModelV4 {
  readonly specificationVersion = "v4" as const;
  readonly provider: string;
  readonly modelId: string;
  readonly supportedUrls: LanguageModelV4["supportedUrls"];

  private readonly configuredOptions: ImpelGatewayModelOptions;
  private readonly gatewayUrl: string;
  private readonly resolved: ResolvedGatewayModel;

  static [WORKFLOW_SERIALIZE](
    model: ImpelGatewayLanguageModel,
  ): SerializedImpelGatewayModel {
    return {
      modelId: model.modelId,
      options: serializeGatewayModelOptions(
        model.configuredOptions,
        model.gatewayUrl,
      ),
    };
  }
  ...
  static [WORKFLOW_DESERIALIZE](
    serialized: SerializedImpelGatewayModel,
  ): ImpelGatewayLanguageModel {
    return new ImpelGatewayLanguageModel(
      serialized.modelId,
      serialized.options,
    );
  }
  ...
  private async buildCall(callOptions: LanguageModelV4CallOptions) {
    const invocation = await resolveGatewayInvocation(
      callOptions,
      this.configuredOptions,
    );
    const configuredHeaders = await resolveSafeConfiguredHeaders(
      this.configuredOptions.headers,
    );
    ...
    return {
      ...
      options: prepareCallOptions(
        callOptions,
        this.configuredOptions.providerOptions,
        invocation.tokensToScrub,
...
async function aggregateStreamResult(
  ...
  let responseMetadata: NonNullable<
  ...
  for await (const part of result.stream) {
    switch (part.type) {
      ...
      case "reasoning-delta": {
        const index = ensureReasoningContent(
          content,
          reasoningIndices,
          part.id,
          part.providerMetadata,
...
function ensureReasoningContent(
  content: LanguageModelV4Content[],
  indices: Map<string, number>,
  id: string,
  providerMetadata: SharedV4ProviderMetadata | undefined,
...
function prepareCallOptions(
  ...
  configuredProviderOptions: SharedV4ProviderOptions | undefined,
  tokensToScrub: readonly string[],
  provider: ImpelGatewayProvider,
...
async function resolveGatewayInvocation(
  ...
  configured: ImpelGatewayModelOptions,
  ...
  const untrustedPromptRunTokens = scanPromptRunTokensForRedaction(
    options.prompt,
  );
  const currentRunToken = promptContexts.currentContext?.runToken;
  if (promptContexts.hasTrustedCurrentContext && !currentRunToken) {
    throw new Error(
      "The trusted current Eve Client context does not contain a runToken. Refusing configured, environment, or PAT fallback.",
    );
  }
  ...
  if (!authToken) {
    throw new Error(
      "An Impel gateway credential is required. Pass clientContext.runToken, configure runContext.runToken or authToken, or set IMPEL_RUN_TOKEN, IMPEL_GATEWAY_TOKEN, or IMPEL_PAT.",
    );
  }
  return {
    authToken,
...
async function resolveConfiguredRunContext(
  provider: ImpelGatewayRunContextProvider | undefined,
): Promise<ImpelGatewayRunContext | null> {
  if (!provider) return null;
  return typeof provider === "function" ? await provider() : provider;
}

async function resolveSafeConfiguredHeaders(
  headers: ImpelGatewayHeaders | undefined,
...
function serializeGatewayModelOptions(
  options: ImpelGatewayModelOptions,
  gatewayUrl: string,
  ...
  const serialized: ImpelGatewayModelOptions = { gatewayUrl };
  ...
  for (const key of [
    ...
    if (typeof options[key] === "string") serialized[key] = options[key];
  }
  if (headers) serialized.headers = sanitizeHeadersInit(headers);
  if (runContext) {
    serialized.runContext = cloneWorkflowJson(runContext, "runContext");
  }
  if (options.providerOptions) {
    serialized.providerOptions = cloneWorkflowJson(
      options.providerOptions,
      "providerOptions",
    );
  }
...
function resolveSynchronousOption<T>(
  ...
  if (isPromiseLike(resolved)) {
    throw new Error(
      `Cannot serialize an Impel gateway model: ${label} returned a Promise. Workflow model serialization only supports synchronous values.`,
    );
  }
  return resolved;
}

function cloneWorkflowJson<T>(value: T, label: string): T {
  if (!isWorkflowJsonSerializable(value)) {
    throw new Error(
      `Cannot serialize an Impel gateway model: ${label} must be JSON-serializable.`,
    );
  }
  return JSON.parse(JSON.stringify(value)) as T;
}

function isWorkflowJsonSerializable(value: unknown): boolean {
  ...
  if (Array.isArray(value)) return value.every(isWorkflowJsonSerializable);
  return (
    typeof value === "object" &&
    Object.getPrototypeOf(value) === Object.prototype &&
    Object.values(value as Record<string, unknown>).every(
      isWorkflowJsonSerializable,
    )
  );
}
...
function sanitizeHeadersInit(headers: HeadersInit): Record<string, string> {
  const safe: Record<string, string> = {};
  new Headers(headers).forEach((value, name) => {
    if (!FORBIDDEN_AUTH_HEADERS.has(name.toLowerCase())) safe[name] = value;
  });
  return safe;
}
...
function scanRunContextsFromPrompt(
  prompt: LanguageModelV4CallOptions["prompt"],
): PackedRunContextScan {
  const userRuns: Array<{ start: number; end: number }> = [];
  ...
  const contextsByRun = userRuns.map(({ start, end }) => {
    const contexts: ImpelGatewayRunContext[] = [];
...
function parseStandalonePackedRunContext(
  message: LanguageModelV4CallOptions["prompt"][number] | undefined,
): ImpelGatewayRunContext | undefined {
  if (
    message?.role !== "user" ||
...
function collectPromptRunTokens(
  value: unknown,
  runTokens: string[],
  ...
  if (typeof value === "string") {
    for (const context of extractEmbeddedClientContexts(value)) {
      for (const key of ["runToken", "identityRunToken"] as const) {
        const runToken = nonEmptyString(context[key]);
        if (runToken) runTokens.push(runToken);
      }
    }
    return;
  }
  ...
  if (Array.isArray(value)) {
    for (const item of value) collectPromptRunTokens(item, runTokens, visited);
    return;
  }
  if (
    Object.getPrototypeOf(value) !== Object.prototype &&
...
function extractEmbeddedClientContexts(text: string): JsonObject[] {
  ...
  while (cursor < text.length) {
    ...
    while (/\s/.test(text[objectStart] ?? "")) objectStart += 1;
    if (text[objectStart] !== "{") {
      cursor = objectStart;
      continue;
    }
...
function normalizePromptRunContext(obj: JsonObject): ImpelGatewayRunContext {
  const installationId =
    typeof obj.installationId === "number" && Number.isFinite(obj.installationId)
...
function scrubPromptRunTokens(
  prompt: LanguageModelV4CallOptions["prompt"],
  runTokens: readonly string[],
): LanguageModelV4CallOptions["prompt"] {
  if (runTokens.length === 0) return prompt;
  return prompt.map((message) => {
    ...
    // intact.
    return scrubPromptValue(message, runTokens) as typeof message;
  });
}

function scrubPromptValue(value: unknown, runTokens: readonly string[]): unknown {
  if (typeof value === "string") return scrubRunTokens(value, runTokens);
  if (Array.isArray(value)) {
...
function scrubRunTokens(text: string, runTokens: readonly string[]): string {
  let scrubbed = text;
...
function mergeProviderOptions(
  ...
  perCall: SharedV4ProviderOptions | undefined,
): SharedV4ProviderOptions {
  const merged: SharedV4ProviderOptions = { ...(configured ?? {}) };
  for (const [namespace, value] of Object.entries(perCall ?? {})) {
    merged[namespace] = {
      ...(asJsonObject(merged[namespace]) ?? {}),
      ...(asJsonObject(value) ?? {}),
    } as SharedV4ProviderOptions[string];
  }
  return merged;
}
...
function mapGatewayError(error: unknown, fallbackModel: string): Error {
  if (error instanceof ImpelGatewayPoolError) return error;
  ...
  if (apiError?.data !== undefined) candidates.push(apiError.data);
  ...
  for (const candidate of candidates) {
    ...
    if (!details) continue;
    const retryAfter =
      details.retryAfter ?? readRetryAfter(apiError?.responseHeaders);
  ...
  if (apiError) {
    const retryableUpstreamAuthRejection =
      ...
      headerValue(apiError.responseHeaders, UPSTREAM_AUTH_REJECTED_HEADER) ===
        "true";
    const retryableManagedVirtualKeyMiss =
      (apiError.statusCode === 401 || apiError.statusCode === 403) &&
...
function gatewayRequestErrorMessage(
...
function poolErrorCode(value: unknown): ImpelGatewayPoolErrorCode | undefined {
  return typeof value === "string" &&
    POOL_ERROR_CODES.has(value as ImpelGatewayPoolErrorCode)
    ? (value as ImpelGatewayPoolErrorCode)
    : undefined;
}
...
function requireGatewayUrl(explicit?: string): string {
  const gatewayUrl = resolveImpelGatewayUrl(explicit);
  if (!gatewayUrl) {
    throw new Error(
      "IMPEL_GATEWAY_URL or gatewayUrl is required to create an Impel gateway model.",
    );
  }
  return gatewayUrl;
}
