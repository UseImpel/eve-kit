...
const inputSchema = z.object({
    query: z
        .string()
        .min(1)
        .describe("A specific, natural-language web search query. Returns real result URLs and a source-backed synthesis; read those, never guess or construct a URL."),
    allowedDomains: z
    ...
    modelId: z
        .string()
        .optional()
        .describe("Override the xAI model. Defaults to XAI_SEARCH_MODEL_ID / XAI_SOCIAL_MODEL_ID / grok-4.20-non-reasoning."),
});
    ...
    async execute(input) {
        ...
        try {
            ...
            return {
                modelId,
                summary: result.text,
                sources: jsonSafe(result.sources ?? []),
                notes: [
            ...
            return {
                modelId,
                summary: "web_search did not complete (aborted after " +
