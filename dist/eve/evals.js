...
export function createImpelBraintrustEvalConfig({ defaultAgentId, defaultProjectName = "Impel", defaultExperimentSuffix = "smoke-v1", }) {
    ...
    const impelBraintrustReporter = Braintrust({
        ...
        experimentName: process.env.BRAINTRUST_EXPERIMENT_NAME ??
...
export function createImpelSmokeEval({ defaultAgentId, defaultOrgId, defaultAgentVersion, defaultAgentDigest = "", message = "Reply briefly: ready.", }) {
    ...
    return defineEval({
        description: "Smoke test that the agent accepts a basic request and completes.",
