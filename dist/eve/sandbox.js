...
export async function installImpelWorkspaceTools(sandbox) {
    const ghCliVersion = process.env.IMPEL_EVE_GH_CLI_VERSION ?? DEFAULT_GH_CLI_VERSION;
    await runBootstrapCommand(sandbox, "install workspace binaries", [
        "set -eu",
...
async function runBootstrapCommand(sandbox, label, command) {
