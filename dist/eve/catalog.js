...
export const EVE_KIT_PROVIDERS = [
    {
        ...
        envVars: [
            ...
            },
            {
                ...
                required: false,
                sensitive: false,
                purpose: "Base URL of the impel-identity service for centralized GitHub token resolution during workspace prep; only used when the dispatcher also sent a signed run token.",
            },
            {
    ...
    },
    {
        ...
        summary: "AI SDK LanguageModelV3 provider that streams through Impel's durable impel-inference service instead of a direct model API. Forwards run context (orgId, repos, branch, installationId, runId/traceId, agent, signed run token) resolved from the prompt sentinel, configured providers, or IMPEL_RUN_* env fallbacks.",
        envVars: [
            {
                name: "IMPEL_INFERENCE_URL",
                required: true,
                sensitive: false,
                purpose: "Base URL of the impel-inference service (or pass baseUrl explicitly).",
            },
            {
                name: "IMPEL_INFERENCE_API_KEY",
                required: true,
                sensitive: true,
                purpose: "API key for impel-inference (or pass apiKey explicitly).",
            },
            {
                name: "IMPEL_ORG_ID",
                required: false,
                sensitive: false,
                purpose: "Fallback org id when no run context supplies one (defaults to 'default').",
            },
        ],
        setupSteps: [
    ...
    },
    {
        ...
        summary: "Canonical Claude model wiring for agent bundles: routes through impelInference when IMPEL_INFERENCE_URL/baseUrl is set, and falls back to the local claude-code CLI provider only in local development (deployed runtimes fail loud instead of hitting a cryptic missing-credential error).",
        envVars: [
            {
                name: "IMPEL_INFERENCE_URL",
                required: false,
                sensitive: false,
                purpose: "Selects the durable impel-inference path; required in deployed runtimes, optional locally where the claude-code CLI fallback works.",
            },
            {
                name: "IMPEL_INFERENCE_API_KEY",
                required: false,
            ...
            },
            {
                ...
                required: false,
                sensitive: false,
                purpose: "Set to '1' to force the local claude-code provider even on a deployed Vercel runtime (escape hatch).",
            },
            {
                name: "IMPEL_ALLOW_LOCAL_PROVIDER_FALLBACK",
                required: false,
                sensitive: false,
                purpose: "Boolean flag gating the local-provider fallback independently of NODE_ENV.",
            },
        ],
        setupSteps: [
    ...
    },
    {
        ...
        summary: "Codex model wiring for agent bundles: always routes through impelInference with the codex-app-server provider and Impel's default approval/sandbox options (createImpelOpenAIResponsesModel is the OpenAI-Responses alias).",
        envVars: [
            {
                name: "IMPEL_INFERENCE_URL",
                required: true,
