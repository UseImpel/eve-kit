...
function resolveConfig(config) {
    ...
    const channelId = config.channelId ?? process.env.WHAPI_CHANNEL_ID ?? PLACEHOLDER;
    if (token === PLACEHOLDER || channelId === PLACEHOLDER) {
        console.warn("[whatsapp] WHAPI_TOKEN / WHAPI_CHANNEL_ID not set — the WhatsApp channel " +
...
class WhapiClient {
    ...
    async request(method, path, body) {
        ...
        if (!response.ok) {
            ...
            throw new Error(`Whapi ${method} ${path} failed: ${response.status} ${detail.slice(0, 300)}`);
        }
    ...
    async sendText(chatId, text) {
        ...
        for (const chunk of splitWhatsAppMessage(text)) {
            const response = await this.request("POST", "/messages/text", {
                to: chatId,
                body: chunk,
            });
    ...
    async sendButtons(chatId, input) {
        const response = await this.request("POST", "/messages/interactive", {
            to: chatId,
            type: "button",
    ...
    async typing(chatId, seconds = 10) {
...
// ---------------------------------------------------------------------------
function initialState(channelId) {
    return { channelId, chatId: null, isGroup: false, pendingFreeform: {} };
}
function stateForChat(channelId, chatId) {
    return {
        channelId,
        chatId,
...
function buildHandle(client, state) {
    ...
    const sendMessage = (text) => client.sendText(requireChat(), renderWhatsAppMarkdown(text));
    return {
...
function defaultEvents(interactiveButtons) {
    return {
        ...
        async "input.requested"(data, channel) {
            for (const req of data.requests ?? []) {
                ...
                if (options.length > 0) {
                    lines.push("", ...options.map((o, i) => `${i + 1}. ${o.label}`), "", "Reply with the number or the option text.");
                }
                await channel.whatsapp.post(lines.join("\n"));
            }
        },
        async "turn.failed"(data, channel) {
            await channel.whatsapp.post(`I hit an error while handling your request${data.message ? ` (${data.message})` : ""}. Please try again or rephrase.`);
        },
        async "session.failed"(data, channel) {
            await channel.whatsapp.post(`This session could not recover from an error${data.message ? ` (${data.message})` : ""}. Send a new message to continue.`);
        },
    };
}
// ---------------------------------------------------------------------------
...
export function whatsappChannel(config = {}) {
    ...
    async function dispatchMessage(message, args) {
        ...
        if (hitl) {
            try {
                await args.send({ inputResponses: [{ requestId: hitl.requestId, optionId: hitl.optionId }] }, { auth: null, continuationToken: continuationToken(message.chatId), state });
            }
            catch (error) {
    ...
    return defineChannel({
        ...
        routes: [
            POST(resolved.route, async (request, args) => {
 ...
 */
export async function sendWhatsAppReply(ctx, markdown) {
