...
test("loadManifest: loads v2 manifest and lazy-loads content", async () => {
    ...
    await writeFile(join(vectorsDir, "finance", "bonds.md.json"), JSON.stringify(bondsVectors));
    const onboardingVectors = [[0.4, 0.5, 0.6]];
    await writeFile(join(vectorsDir, "ops", "onboarding.md.json"), JSON.stringify(onboardingVectors));
    // Load the manifest
...
test("loadManifest: rejects wrong version", async () => {
    ...
    try {
        ...
        assert(e.message.includes("Unsupported manifest version"));
    }
});
