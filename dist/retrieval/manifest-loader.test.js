...
async function writeVault(root) {
    ...
    const sidecarAbs = join(root, "wiki", "_meta", "embeddings", sidecarFileName(bondsHash, MODEL, DIMS));
    await mkdir(dirname(sidecarAbs), { recursive: true });
    await writeFile(sidecarAbs, serializeSidecar({
        manifest: {
            modelId: MODEL,
            dimensions: DIMS,
            passageCount: 1,
...
test("loadManifest: accepts entries under docs when the pages mirror is absent", async () => {
    ...
    await writeFile(manifestPath, JSON.stringify({ ...raw, docs: raw.pages, pages: undefined }));
    const loader = await loadManifest({ manifestPath });
    assert.equal(loader.manifest.pages.length, 2);
});
test("loadManifest: throws loudly on a missing manifest", async () => {
    const root = await mkdtemp(join(tmpdir(), "manifest-test-"));
    await assert.rejects(loadManifest({
...
test("loadManifest: rejects wrong version", async () => {
    ...
    await assert.rejects(loadManifest({ manifestPath, baseDir: root }), /unsupported manifest version/);
});
test("loadManifest: reads a sidecar stored as base64 text", async () => {
    ...
    const sidecarAbs = join(root, "wiki", "_meta", "embeddings", sidecarFileName(sha256("A bond pays periodic interest."), MODEL, DIMS));
...
test("loadManifest: skips a page listed in the manifest but missing on disk", async () => {
    const root = await mkdtemp(join(tmpdir(), "manifest-test-"));
    const manifestPath = await writeVault(root);
    await (await import("node:fs/promises")).rm(join(root, "wiki", "ops", "onboarding.md"));
    const loader = await loadManifest({ manifestPath });
    const docs = await loader.toPageDocs();
    assert.equal(docs.length, 1);
    assert.equal(docs[0].path, "wiki/finance/bonds.md");
});
//# sourceMappingURL=manifest-loader.test.js.map