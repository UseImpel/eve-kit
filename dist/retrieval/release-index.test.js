...
async function writeEmittedVault(root) {
    ...
    const sidecarRel = sidecarFileName(bondsHash, EMBEDDING_MODEL, EMBEDDING_DIMENSIONS);
...
test("loadReleaseIndex hard-errors on a missing manifest — no fallback to index.json", async () => {
    ...
    try {
        ...
        await writeFile(join(root, "wiki", "_meta", "index.json"), JSON.stringify({ version: 1, docs: [] }));
        await assert.rejects(loadReleaseIndex({
...
test("loadReleaseIndex hard-errors on a corrupt manifest", async () => {
    ...
    try {
        ...
        await assert.rejects(loadReleaseIndex({ path: manifestPath, embedder: fakeEmbedder() }), /unreadable/);
    }
    finally {
        await rm(root, { recursive: true, force: true });
    }
...
test("loadReleaseIndex backfills when page content drifted from its contentHash", async () => {
    ...
    try {
        ...
        await writeFile(join(root, "wiki", "finance", "bonds.md"), "A bond pays a coupon. Yields move inversely to price.");
        const { stats } = await loadReleaseIndex({
            path: manifestPath,
            embedder: fakeEmbedder(),
        });
