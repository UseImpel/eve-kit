...
function resolveV2ManifestPath(v1IndexPath) {
...
async function loadV1Index(opts) {
    ...
    if (index.model !== EMBEDDING_MODEL ||
        ...
        console.warn(`[release-index] index embedding ${index.model}@${index.dimensions} ` +
            `differs from retrieval default ${EMBEDDING_MODEL}@${EMBEDDING_DIMENSIONS} — ` +
...
function stripToEmbedded(doc, embedding) {
    return {
        path: doc.path,
        title: doc.title,
        content: doc.content,
