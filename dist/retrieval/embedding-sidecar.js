...
export function sidecarFileName(contentHash, modelId, dimensions) {
...
export function serializeSidecar(sidecar) {
    ...
    const vectorArray = new Float32Array(sidecar.vectors.length * sidecar.manifest.dimensions);
    let idx = 0;
...
export function deserializeSidecar(buffer) {
    if (buffer.length < 4) {
    ...
    if (4 + manifestLength > buffer.length) {
        throw new Error(`deserializeSidecar: manifest length ${manifestLength} exceeds buffer (total ${buffer.length})`);
    }
    const manifest = JSON.parse(buffer.toString("utf-8", 4, 4 + manifestLength));
    ...
    if (vectorStartByte + vectorBytesNeeded !== buffer.length) {
        throw new Error(`deserializeSidecar: vector data size mismatch (expected ${vectorBytesNeeded}, got ${buffer.length - vectorStartByte})`);
    }
    ...
    for (let i = 0; i < manifest.passageCount; i++) {
        ...
        for (let j = 0; j < manifest.dimensions; j++) {
            vector[j] = buffer.readFloatLE(vectorStartByte + (i * manifest.dimensions + j) * 4);
        }
        vectors.push(vector);
    }
    return { manifest, vectors };
}
//# sourceMappingURL=embedding-sidecar.js.map