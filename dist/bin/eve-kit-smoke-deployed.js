#!/usr/bin/env node
import { ImpelRuntimeSmokeError, smokeDeployedRuntime, } from "../eve/runtime.js";
const baseUrl = (process.argv[2] ?? process.env.EVE_APP_URL ?? "").replace(/\/$/, "");
if (process.argv.includes("--help") || process.argv.includes("-h")) {
    console.log("Usage: EVE_APP_URL=https://... eve-kit-smoke-deployed\n" +
...
try {
    ...
    console.log(`[smoke] ok sessionId=${result.sessionId} events=${result.eventCount} textBytes=${result.textBytes} completed=${result.completed}`);
}
catch (error) {
    const message = error instanceof ImpelRuntimeSmokeError || error instanceof Error
        ? error.message
        : String(error);
    console.error(`[smoke] ${message}`);
    process.exit(1);
}
//# sourceMappingURL=eve-kit-smoke-deployed.js.map